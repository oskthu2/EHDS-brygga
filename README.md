...
| **GetCareDocumentation: DocBook-detektion** | `clinicalDocumentNoteText` som ser ut som DocBook-XML transformeras via `DocBookToNarrativeTransformer` till både `text/html` (Strategy A) och en separat `Composition` med `Composition.section` per DocBook-sektion (Strategy B); detektionen är en enkel heuristik (börjar med `<`), inte en spec-fastställd regel. `Composition`s koppling tillbaka till `DocumentReference` går via delad `Provenance`, inte ett eget FHIR-fält, och löst innehåll utanför strukturella sektioner hamnar i en avslutande sektion snarare än sin ursprungliga position. Se DOC-004. | `GetCareDocumentationMapper.java`, `DocBookToNarrativeTransformer.java` |
